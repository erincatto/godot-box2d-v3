...
Box2DGrooveJoint2D::Box2DGrooveJoint2D(const Vector2 &p_a_groove1, const Vector2 &p_a_groove2, const Vector2 &p_b_anchor, Box2DBody2D *p_body_a, Box2DBody2D *p_body_b) :
	...
	// Body B slides along the x axis of frame A, so the groove direction is the frame rotation.
	wheel_def.base.localFrameA.q = b2MakeRotFromUnitVector(to_box2d_normalized(axis));
